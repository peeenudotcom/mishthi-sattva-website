...
/* ---------------- product detail sections ----------------
   ...
   to look: Description → Wellness Benefits → Ingredients → Nutritional
   Information → Storage & Shelf Life → Our Promise → Customer Reviews.
   A section that has no data simply doesn't render — no empty headings. */
      ...
      {items.map((b, i) => {
        ...
        return (
          <li key={i} style={{ display: "flex", gap: 10 }}>
            <span style={{ color: "var(--accent)", marginTop: 2, flexShrink: 0 }}><I.leaf s={16} /></span>
            <span>
    ...
    return (
      <div>
        ...
        <a href={"https://wa.me/" + ((window.MS_CONFIG && window.MS_CONFIG.WHATSAPP) || "") + "?text=" + encodeURIComponent("Hi! I'd like to leave a review for " + product.name)}
          ...
          <I.wa s={16} /> Share your experience
        </a>
      </div>
    );
  }
          ...
          <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "grid", gap: 10 }}>
            {promise.map((p) => (
              <li key={p} style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 14, lineHeight: 1.6, color: "color-mix(in oklab, var(--foreground) 82%, transparent)" }}>
                <span style={{ color: "var(--accent)", marginTop: 3, flexShrink: 0 }}><I.check s={15} /></span>{p}
              </li>
            ))}
          </ul>
        </Section>
