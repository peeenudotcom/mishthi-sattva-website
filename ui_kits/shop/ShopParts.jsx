...
/* ---------------- product detail sections ----------------
   The seven-section product story lives in ui_kits/shared/ProductDetails.jsx
   because the website's product popup shows exactly the same thing. Read it
   off the window at render time so load order doesn't matter. */
  ...
  return (
    <Overlay onClose={onClose} align="center">
      <div onClick={(e) => e.stopPropagation()} style={{ width: "min(880px, 94vw)", maxHeight: "90vh", overflow: "auto", background: "var(--card)", borderRadius: "var(--radius-3xl)", boxShadow: "var(--shadow-xl)", border: "1px solid var(--border)" }}>
        {/* Columns come from the .shop-quick stylesheet rule, NOT an inline
            style — inline would outrank the media query and keep the modal
            two-column on phones (it did, until this was removed). */}
        <div className="shop-quick" style={{ display: "grid" }}>
          {/* Sticky so the photo stays with you while you read the detail
              sections below — the column would otherwise scroll away blank. */}
          <div className="shop-quick-media" style={{ padding: 24, position: "sticky", top: 0, alignSelf: "start" }}><ProductMedia product={product} height={360} round={24} /></div>
          <div className="shop-quick-info" style={{ padding: "32px 32px 32px 8px", position: "relative" }}>
            ...
            {/* Only the short blurb here. When a full Product Description
                exists it opens as the first detail section below, and
                showing both read as the description repeating itself. */}
            {!product.long_desc && (
...
window.MSShopParts = { I, Stars, money, catName, catTint, ProductMedia, ProductCard, Stepper, QuickView, Overlay, TagPill, ProductDetails };
})();
